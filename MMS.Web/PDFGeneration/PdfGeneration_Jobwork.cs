...
{
    public class PdfGeneration_Jobwork
    {
        public static string PrintPdfGeneration_Jobwork(string GateEntryNo, string contents)
        {
         ...
         JobworkMaster JobworkMaster = new JobworkMaster();
            ...
            {
                receiptDetailslist = manager.GeneratePdf_Jobwork(values[i]);
                int pagebreak = 0;
                ...
                if (receiptDetailslist != null && receiptDetailslist.Count > 0)
                {
                    ...
                    for (int j = 1; j <= tempreceiptDetailslist; j++)
                    {
                        ...
                        {
                            invoiceDynamicRow += GetReceiptItems_jobworl(item, count_);
                        ...
                        if (SupplierId != 0)
                        {
                            JobworkMaster = jobworkManager.GetJobMasterMasterID(SupplierId);
                            //supplierMaster = GetSupplier(SupplierId);
                        }
                        string aa = JobworkMaster.Address;
                        string ab = "";
                        string ac = "";
                        string GSTIN = JobworkMaster.Cst_No_Head;
                        HtmlText = HtmlText.Replace("[[SupplierName]]", string.IsNullOrEmpty(JobworkMaster.JW_Name) ? " " : JobworkMaster.JW_Name);
        ...
        public static string GetReceiptItems(GeneratePdf receiptDetails, int count_)
        {
            ...
            if (outMaster.Count != 0)
            {
                ...
                invoiceDynamicRow += "<tr>" +
                                          "<td style='border-bottom: 1px solid #333;border-right: 1px solid #333;font-size:8px;font-weight:normal;font-size:10px;text-align:center;' cellpadding='5'>" + count_ + "</td>" +
                                           "<td style='border-bottom: 1px solid #333;border-right: 1px solid #333;font-size:8px;font-weight:normal;font-size:10px;text-align:center;' cellpadding='5'></td>" +
                                          "<td style='border-bottom: 1px solid #333;border-right: 1px solid #333;font-size:8px;font-weight:normal;font-size:10px;text-align:center;' cellpadding='5'>" + HSNCode + "</td>" +
                                          "<td style='border-bottom: 1px solid #333;border-right: 1px solid #333;font-size:8px;font-weight:normal;font-size:10px;text-align:center;' cellpadding='5'>" + Rate + "</td>" +
                                          "<td style='border-bottom: 1px solid #333;border-right: 1px solid #333;font-size:8px;font-weight:normal;font-size:10px;text-align:center;' cellpadding='5'>" + UomName + "</td>" +
                                           "<td style='border-bottom: 1px solid #333;border-right: 1px solid #333;font-size:8px;font-weight:normal;font-size:10px;text-align:center;' cellpadding='5'>" + Piecies + "</td>" +
        ...
        public static string GetReceiptItems_jobworl(GeneratePdf receiptDetails, int count_)
        {
            ...
            string MaterialName = receiptDetails.MaterialNameId.ToString();
            if (receiptDetails.Purpose.ToString() != "PRODUCTION JOBWORK AND RETURNABLE")
            {
                if (MaterialName != null)
                {
                    outMaster = GetMaterialName_Jobwork(MaterialName);
                }
            }
            else
            {
                if (MaterialName != null)
                {
                    outMaster = ComponentManager.Get().Where(x => x.ComponentMasterId == Convert.ToInt32(MaterialName)).Select(x => new MMS.Web.Models.GateEntryModel.MaterialMaster
                    {
                        MaterialNameId = x.ComponentMasterId,
                        MaterialDescription=x.ComponentName,
                        MaterialMasterId = x.ComponentMasterId,
                        MaterialName=x.ComponentName,
                    }).ToList();
                }
            }
        ...
        public static List<MMS.Web.Models.GateEntryModel.MaterialMaster> GetMaterialName_Jobwork(string MaterialName)
        {
            List<MMS.Web.Models.GateEntryModel.MaterialMaster> supplierlist = new List<MMS.Web.Models.GateEntryModel.MaterialMaster>();
            using (var context = new MMSContext())
            {
                supplierlist = context.Database.SqlQuery<MMS.Web.Models.GateEntryModel.MaterialMaster>("EXEC GetOutwardGateEntry_DcReport @IssueSlipNo={0}", MaterialName).ToList();
